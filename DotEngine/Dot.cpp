...
void Dot::Render(DotRenderer* aRenderer, float dt)
{
	...
	float redColor = (glm::cos((m_TotalTime + m_Position.x) * 0.1f) * 0.5f + 0.5f) * 255.0f;

	float greenColor = (glm::cos((m_TotalTime + m_Position.y) * 0.9f) * 0.5f + 0.5f) * 255.0f;

	float blueColor = (glm::cos(m_TotalTime * 0.4f) * 0.5f + 0.5f) * 255.0f;
	
	aRenderer->DrawFilledCircle(m_Position.x, m_Position.y,
		m_Radius, redColor, blueColor, greenColor);
